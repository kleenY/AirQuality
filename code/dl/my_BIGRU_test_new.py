# -*- coding: utf-8 -*
# @Time : 2024/11/23 18:16
# @Author : 杨坤林
# @File : my_BIGRU_test.py
# @Software : PyCharm
import os
import pickle
import numpy as np
...
class my_BIGRU(nn.Module):
    def __init__(self,
        ...
        self.bigru = nn.GRU(input_size=self.input_size, hidden_size=self.hidden_size,
                            num_layers=self.num_layers, dropout=self.dropout, batch_first=self.batch_first,
                            bidirectional=self.bidirectional)

        self.reg = nn.Sequential(nn.Linear(self.hidden_size * 2, self.hidden_size),  # 修改线性层输入大小以适应双向GRU输出
                                 nn.Tanh(),
    ...
    def forward(self, x):
        ...
        bigru_out, hidden = self.bigru(x, hidden_state)
        outputs = self.reg(bigru_out)
        # print(outputs.shape)
...
def my_test_model(model_save_path, pkl_file_path, scaler_X_path, scaler_y_path, future_hours=1,
                  output_dir="BIGRU_results", batch_size=32, target_column='CO(GT)'):
    # 创建输出目录
    ...
    model = my_BIGRU(input_size=len(selected_features), output_size=output_size, seq_len=seq_len, pred_len=pred_len,
    ...
    metrics_path = os.path.join(output_dir, "BIGRU_test_metrics.csv")
    ...
    # 添加图例和标题
    plt.legend()
    plt.title("Air Quality Prediction (BIGRU Test Samples)")
    ...
    # 保存图像
    plot_path = os.path.join(output_dir, 'BIGRU_prediction_test_samples.png')
    plt.savefig(plot_path)
    print(f"Prediction plot saved to '{plot_path}'")

    # 保存所有样本的数据到CSV文件
    data_df = pd.DataFrame(all_data)
    csv_path = os.path.join(output_dir, 'BIGRU_prediction_test_samples_data.csv')
...
if __name__ == '__main__':
    ...
    for data_type in data_types:
        for target_column in target_columns:
            ...
            model_save_path = f'./{target_column}_{data_type}_BIGRU_output/best_BIGRU_model.pth'
            scaler_X_path = f'./{target_column}_{data_type}_BIGRU_output/scaler_X.pkl'
            scaler_y_path = f'./{target_column}_{data_type}_BIGRU_output/scaler_y.pkl'
            output_dir = f'{target_column}_{data_type}_BIGRU_results'
