...
def my_train_3(pkl_file_path, output_dir='BIGRU_output', step=1, weight_decay=1e-4, target_column = 'CO(GT)'):
    # 创建输出目录
    ...
    model = my_BIGRU(input_size=input_size, output_size=output_size, seq_len=seq_len, pred_len=pred_len, device=device).to(device)
    # criterion = nn.functional.smooth_l1_loss()
    optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=weight_decay)  # 加入正则化

    best_val_mae = float('inf')
    model_save_path = os.path.join(output_dir, 'best_BIGRU_model.pth')
    ...
    for epoch in range(num_epochs):
        ...
        # # 每隔几个 epoch 保存一次模型
        # if (epoch + 1) % 100 == 0:
        #     epoch_model_path = os.path.join(output_dir, f'BIGRU_model_epoch_{epoch+1}.pth')
    ...
    losses_csv_path = os.path.join(output_dir, 'BIGRU_losses.csv')
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.title("BIGRU Train and Validation Loss Over Epochs")
    plt.legend()
    loss_plot_path = os.path.join(output_dir, "BIGRU_train_val_loss.png")
...
if __name__ == '__main__':
    ...
    # target_column = 'CO(GT)'
    # my_train_3(pkl_file_path=pkl_file_path, output_dir =target_column +'_long_BIGRU_output', target_column=target_column)
    ...
    for data_type in data_types:
        for target_column in target_columns:
            ...
            output_dir = f'{target_column}_{data_type}_BIGRU_output'
