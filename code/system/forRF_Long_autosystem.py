...
for label in labels:
    ...
    for mode, (input_size, predict_size) in split_modes.items():
        ...
        features = ['NOx(GT)', 'NO2(GT)', 'CO(GT)', 'PT08.S5(O3)']
        ...
        #print(f"X_test:{X_test},X_test_shape:{X_test.shape}")
        #print("-----------------------------------------")
        #print(f"X_test_flat:{X_test_flat},X_test_flat_shape:{X_test_flat.shape}")
        ...
        # 保存拼接后训练的模型
        model_filename = f"./model/random_forest_{mode}_{label}_model_combined.pkl"
        joblib.dump(model, model_filename)

        """# 保存评价指标
        metrics_filename = f"./model/RF_{mode}_{label}_metrics.txt"
        ...
        csv_filename = f"{model_dir}/random_forest_{Flag}_{label}_prediction_data.csv"
        ...
        plt.ylabel(f'{label}')
        plt.title(f'Air Quality Prediction - RF  - {Flag}')
        plt.legend()
        plt.grid(True)
        plot_filename = f"{model_dir}/RF_{Flag}_{label}_prediction_plot.png"
        ...
        shap_values = explainer(X_test_flat_np, check_additivity=False)
        ...
        # 保存 SHAP 值图
        shap_plot_filename = f"{model_dir}/RF_{Flag}_{label}_shap_plot.png"
